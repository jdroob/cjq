...
json_file1="$HOME/cjq/cjq/tests/basic_ops/json/prod_example/prod1.json"
json_file2="$HOME/cjq/cjq/tests/basic_ops/json/prod_example/prod2.json"

# Function to compare outputs and write result to testresults.log
function compare_outputs {
    if [ "$1" = "$2" ]; then
        echo "PASSED : $3" >> cjq/tests/basic_ops/testresults.log
        ((passed_tests++))
    else
        echo "FAILED : $3" >> cjq/tests/basic_ops/testresults.log
        echo "Differences in output for $3:" >> cjq/tests/basic_ops/testresults.log
        diff <(echo "$1") <(echo "$2") >> cjq/tests/basic_ops/testresults.log
        ((failed_tests++))
    fi
    ((total_tests++))
}

# Command 0: Clear tests log
echo "" > cjq/tests/basic_ops/testresults.log

# Print message indicating tests below opcode functions
echo "tests builtin operators and functions:"
# echo "tests BELOW OPCODE FUNCTIONS:"
# echo -e "\nTOP"
# echo "SUBEXP_BEGIN"
# echo "SUBEXP_END"
# echo "PUSHK_UNDER"
...
for jq_file in cjq/tests/basic_ops/jq/prod_example/*.jq; do
    ...
    # Compare outputs and write result to tests.log
...
for test_case in "${test_cases[@]}"; do
    jq_file="$HOME/cjq/cjq/tests/basic_ops/jq/builtin_ops/${test_case}.jq"
    json_file="$HOME/cjq/cjq/tests/basic_ops/json/builtin_ops/${test_case}.json"
...
for test_case2 in "${test_cases2[@]}"; do
    ...
    # Command 2: Run runjq and capture output
    # echo "Running cjq for $jq_file..."  # Debug
    cjq_output=$(./run -n "$test_case2" --debug-dump-disasm)
